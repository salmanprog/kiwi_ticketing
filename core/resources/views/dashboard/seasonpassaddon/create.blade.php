...
@push("after-styles")
    ...
    <style>
        ...
        body.modal-open .padding, body.modal-open .navbar, body.modal-open .sidebar, body.modal-open .app-aside {
            filter: blur(5px);
        ...
        <div class="box">
            ...
            <div class="box-body" style="display: none;">
                ...
                <button type="button" class="btn btn-primary" id="openProductModal">
                    <i class="material-icons">&#xe02e;</i> {{ __('Create New Product') }}
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="custom-modal" id="productModal">
        <div class="custom-modal-content">
            <div class="custom-modal-header">
                ...
                <button type="button" class="close-btn" id="closeProductModal" title="Back to previous page">&times;</button>
            </div>
            <div class="custom-modal-body">
                ...
                {{ Form::open(['route' => ['seasonpassaddonStore'], 'method' => 'POST', 'files' => true, 'id' => 'multiStepForm']) }}

                <!-- Step 1: Product Information -->
                <div class="step active" id="step1">
                    <div class="form-group row mb-3">
                        <label for="season_passes_slug" class="col-sm-3 form-control-label">
                            {!! __('Sale Pass') !!} <span class="text-danger">*</span>
                        </label>
                        <div class="col-sm-9">
                            <select name="season_passes_slug" class="form-control" required>
                                <option value="">- - {!!  __('Select Sale Pass') !!} - -</option>
                                @foreach($getSeasonPass as $pass)
                                    <option value="{{ $pass['slug'] }}">{{ $pass['title'] }}</option>
                                @endforeach
                            </select>
                            <div class="invalid-feedback">Please select sale pass</div>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="ticketSlug" class="col-sm-3 form-control-label">
                            {!! __('Sale Product') !!} <span class="text-danger">*</span>
                        </label>
                        <div class="col-sm-9">
                            <select name="ticketSlug" id="ticketSlug" class="form-control" required>
                                <option value="">- - {!!  __('Select Sale Product') !!} - -</option>
                                @foreach($addon_arr as $addon)
                                    <option value="{{ $addon['ticketSlug'] }}" data-price="{{ $addon['price'] ?? '0' }}">{{ $addon['ticketType'] }}</option>
                                @endforeach
                            </select>
                            <div class="invalid-feedback">Please select sale product</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            @foreach(Helper::languagesList() as $ActiveLanguage)
                                @if($ActiveLanguage->box_status)
                                    <div class="mb-3">
                                        <label class="form-label">{!!  __('backend.customFieldsType99') !!} {!! @Helper::languageName($ActiveLanguage) !!}</label>
                                        <div class="box p-a-xs">
                                            <textarea name="description_{{ $ActiveLanguage->code }}" 
                                                class="form-control summernote_{{ $ActiveLanguage->code }}"
                                                dir="{{ $ActiveLanguage->direction }}" 
                                                rows="5"
                                                placeholder="Enter description in {{ $ActiveLanguage->name }}"></textarea>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
                ...
                <div class="step" id="step3">
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="new_price" placeholder="Enter price" class="form-control" id="new_price" required>
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-sm-9">
                            <input type="file" name="photo[]" class="form-control" id="photo" accept="image/*" multiple>
                        </div>
                    </div>
                    <div class="form-group row">
                ...
                <div class="step" id="step4">
                    <div class="form-group row mb-3">
                        <label class="col-sm-3 form-control-label">Featured Product</label>
                        <div class="col-sm-9">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="is_featured" id="is_featured1" value="1" checked>
                    ...
                    <div class="form-group row mb-3">
                        ...
                        <div class="col-sm-9">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="is_new_price_show" id="is_new_price_show1" value="1" checked>
                                <label class="form-check-label" for="is_new_price_show1">Yes</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="is_new_price_show" id="is_new_price_show2" value="0">
                ...
                <div class="form-navigation mt-4">
                    <button type="button" class="btn btn-secondary" id="prevBtn" onclick="nextPrev(-1)" style="display: none;">
                    ...
                    <button type="submit" class="btn btn-success" id="submitBtn" style="display: none;">
                        <i class="material-icons">&#xe161;</i> {!! __('Create Product') !!}
                    </button>
                </div>
