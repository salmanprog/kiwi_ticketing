...
<div class="box">
    <div class="box-header dker">
        <h3>{{ __('Product Sale Addon') }}</h3>
        <small>
            <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
            <a href="">{{ __('product-sale-addon') }}</a>
        </small>
    </div>
     <div class="row p-a pull-right" style="margin-top: -70px;">
        <div class="col-sm-12">
            <a class="btn btn-fw primary" href="{{route('productsaleaddonCreate')}}">
                <i class="material-icons">&#xe7fe;</i>
                &nbsp; {{ __('Add Product Sale Addon') }}
            </a>
        </div>
    </div>
    
        <div class="table-responsive">
                    <table class="table table-bordered m-a-0" id="product_sale_addon">
                        <thead class="dker">
                        <tr>
                            <th>{{ __('venueId') }}</th>
                            <th>{{ __('Offer') }}</th>
...
<script src="{{ asset('assets/dashboard/js/datatables/datatables.min.js') }}"></script>
    <script type="text/javascript">
        ...
        function DeleteTicketAddon(id) {
            let url = '{{ route("productsaleaddonDestroy", ":id") }}';
        ...
        $(document).ready(function () {
            var dataTable = $("#product_sale_addon").DataTable({
                processing: true,
                serverSide: true,
                searching: true,
                ajax: {
                    url: "{{ route('productsaleaddon.data') }}",
                    type: "POST",
                    data: function (data) {
               ...
               dom: '<"row"<"col-sm-6"f><"col-sm-6"l>>rtip',
                ...
                initComplete: function () {
                    ...
                    var $searchInput = $('#product_sale_addon_filter input');
                    $searchInput.attr('placeholder', 'Search by offer');
                }
            });
