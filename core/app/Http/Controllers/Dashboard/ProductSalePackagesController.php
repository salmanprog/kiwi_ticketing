...
class ProductSalePackagesController extends Controller
{
    ...
    public function index()
    {
        ...
        $offer_creation_packages = ProductSale::with(['media_slider','createdBy','updatedBy'])->where('auth_code',$authCode)->get();
         // Paginate
        $perPage = 10;
        ...
        return view("dashboard.product_sale_packages.list", compact("paginated", "GeneralWebmasterSections"));
    }

    public function getData(Request $request)
    {
        $authCode = Helper::GeneralSiteSettings('auth_code_en');
        $query = ProductSale::with(['media_slider','createdBy','updatedBy'])->where('auth_code',$authCode);
        ...
        foreach ($data as $row) {
            $result[] = [
                ...
                'title' => '<a class="dropdown-item" href="' . route('productsalecreationpackagesEdit', $row->slug) . '">'.$row->title.'</a>',
                'offerType' => '<a class="dropdown-item" href="' . route('productsalecreationpackagesEdit', $row->slug) . '">'.($row->offerType == 'any_day') ? 'Any Day' : 'Specific Date'.'</a>',
                'slug' => $row->slug,
                ...
                'options' => '<div class="dropdown">
                                ...
                                <div class="dropdown-menu pull-right">
                                    <a class="dropdown-item" href="' . route('productsalecreationpackagesEdit', $row->slug) . '">
    ...
    public function create()
    {
        ...
        $GeneralWebmasterSections = WebmasterSection::where('status', '=', '1')->orderby('row_no', 'asc')->get();
        // General END
        return view("dashboard.product_sale_packages.create", compact("GeneralWebmasterSections"));
    }
    ...
    public function store(Request $request)
    {
        ...
        $productSalePackage = new ProductSale;
        $productSalePackage->auth_code  = $authCode;
        $productSalePackage->title  = $request->title;
        $productSalePackage->offerType  = $request->offerType;
        $productSalePackage->description = $request->description;
        $productSalePackage->from_date = $request->from_date;
        $productSalePackage->to_date = $request->to_date;
        $productSalePackage->created_by = Auth::user()->id;
        $productSalePackage->status = $request->status;
        $productSalePackage->save();

        if(count($uploadedFileNames) > 0){
            for($i=0;$i<count($uploadedFileNames);$i++){
                $media = new Media;
                $media->module  = 'product_sale_packages';
                $media->module_id = $productSalePackage->id;
        ...
        return redirect()->action('Dashboard\ProductSalePackagesController@index')->with('doneMessage', __('backend.addDone'));
    }
    ...
    public function edit($slug)
    {
        ...
        $authCode = Helper::GeneralSiteSettings('auth_code_en');
        $GeneralWebmasterSections = WebmasterSection::where('status', '=', '1')->orderby('row_no', 'asc')->get();
        $offer_creation_packages = ProductSale::with(['media_slider'])->where('slug',$slug)->where('auth_code',$authCode)->first();
        return view("dashboard.product_sale_packages.edit", compact("offer_creation_packages", "GeneralWebmasterSections"));
    }

    public function update(Request $request, $id)
    {
       $authCode = Helper::GeneralSiteSettings('auth_code_en');
       $offerCreationPackages = ProductSale::where('slug',$id)->where('auth_code',$authCode)->first();
       if (!empty($offerCreationPackages)) {
            ...
            return redirect()->action('Dashboard\ProductSalePackagesController@edit', [$id])->with('doneMessage',
                __('backend.saveDone'));

       }else{
            return redirect()->action('Dashboard\ProductSalePackagesController@index');
       }
    }
